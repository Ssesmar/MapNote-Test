...
function Addon:PLAYER_LOGIN() -- OnInitialize()
  ...
  function ns.RemoveBlizzPOIs()
    ...
    for pin in WorldMapFrame:EnumeratePinsByTemplate("AreaPOIPinTemplate") do
      ...
      for pin in WorldMapFrame:EnumeratePinsByTemplate("MapLinkPinTemplatedo") do

        for _, poiID in pairs(ns.BlizzAreaPoisInfo) do

          local UiMap = C_AreaPoiInfo.GetAreaPOIInfo(WorldMapFrame:GetMapID(), pin.linkedUiMapID)
          if (UiMap ~= nil and UiMap.linkedUiMapID == poiID) then
              WorldMapFrame:RemovePin(pin)
          end
        end
      end
        ...
        for i,v in pairs(node.id) do
          local name = dungeonInfo(v)
            ...
            if (node.name) then
              ...
              if not ns.Addon.db.profile.TooltipInformations then
                node.name = name
              elseif ns.Addon.db.profile.TooltipInformations then
